...
import { getManufacturingProcesses } from 'apiSdk/manufacturing-processes';
...
import { ShipmentListPage as ShowCaseList } from 'pages/shipments';

function DashboardPage() {
  const { session } = useSession();

  const manufacturingProcessStat = useSWR<PaginatedInterface<ManufacturingProcessInterface>>(
    () => `/manufacturing-processes`,
    () => getManufacturingProcesses({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      ...
      result: manufacturingProcessStat?.data?.totalCount,
      isLoading: manufacturingProcessStat.isLoading,
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`shipments`} passHref legacyBehavior>
                  <Button
